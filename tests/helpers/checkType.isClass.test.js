...
test('Positive: isClass with class input @examples', function () {
    tester.test(class A {}, true);
});

test('Negative: isClass with no-class input @examples', function () {
    tester.test(13, false);
    tester.test(true, false);
    tester.test(new Set(), false);
});

test('Negative: isClass with empty input @examples', function () {
