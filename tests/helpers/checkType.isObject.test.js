...
test('Positive: isObject with object input @examples', function () {
    tester.test({}, true);
    tester.test(new Object(), true); // eslint-disable-line no-new-object
});

test('Negative: isObject with no-object input @examples', function () {
    tester.test(13, false);
    tester.test(true, false);
    tester.test(new Set(), false);
});

test('Negative: isObject with empty input @examples', function () {
